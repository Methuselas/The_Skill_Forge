...
4. **Set the truth contract and evidence architecture.** Activate `writing_creative_nonfiction_preserve_factual_accountability_while_using_literary_craft`. Inventory direct memory, observation, interviews, records, correspondence, artifacts, prior accounts, inference, reconstruction, dispute, and unknowns. Then activate `writing_creative_nonfiction_verify_checkable_facts_and_research_necessary_context` to test source quality, archive asymmetry, missing voices, and research tasks. When a photograph carries a consequential event or gap, run `writing_creative_nonfiction_drill_interrogate_photograph_as_bounded_evidence`; when a remembered episode already arrives as a smooth story, run `writing_creative_nonfiction_drill_compare_associative_and_narrative_recall` to expose omitted material and implied causality. Define how speculation and reconstruction will be signaled. **Evidence gate:** central claims have supportable routes, and no structural promise depends on an absence being filled by invention.

...
## Steps / Flow
1. **Name the scene change.** Start with the speakers, immediate situation, and what must be different by the end: knowledge, power, commitment, relation, or available action. Preserve that intended change and each character's established motives as invariants. **Advance gate:** the conversation has a job beyond displaying voice.
2. **Set the social conditions.** Activate `writing_fiction_let_relationship_and_setting_shape_dialogue`. Record who is speaking to whom, relative power and intimacy, location, occasion, witnesses, and risk. If the same exchange would fit any pair or place, revise the conditions before drafting lines.
3. **Choose the delivery branch.** Activate `writing_fiction_choose_direct_or_reported_dialogue_by_effect`. Use direct speech where timing, exact wording, or vocal presence carries force; report what needs compression or a revealing narrator filter. If neither form earns the passage, omit the exchange and narrate only its consequence.
4. **Draft the verbal action.** Activate `writing_fiction_compress_real_speech_into_purposeful_dialogue`. Write speech-like rhythms and fragments, then test each turn for what it reveals or changes. **Advance gate:** no retained exchange consists only of routine talk, shared exposition, or polished authorial sentences.
5. **Expose the unsaid.** Activate `writing_fiction_combine_dialogue_with_action_thought_and_silence`. Add only those pauses, gestures, thoughts, and actions that contradict, conceal, intensify, or redirect the words. If beats merely decorate every line, remove them; if a wall of speech erases bodies and subtext, restore them at pressure points.
6. **Orient the reader.** Activate `writing_fiction_orient_speakers_with_unobtrusive_formatting_and_tags`. Give each speaker a paragraph, establish turn order, and retag after ambiguity-producing interruptions or added speakers. **Advance gate:** a cold reader can assign every line without noticing attribution as a competing performance.

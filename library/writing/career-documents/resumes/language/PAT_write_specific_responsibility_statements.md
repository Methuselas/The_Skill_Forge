---
object_id: writing_write_specific_responsibility_statements
object_type: pattern
name: Write Specific Responsibility Statements
library_path:
  - writing
  - career-documents
  - resumes
  - language
stage_binding: 3 rough
lane_fit: skill
foundation_role: specialization
routing_class: specialized
specialization_axis: genre
foundation_object_id: none
tags:
  - resume_writing
  - responsibilities
  - action_verbs
  - specificity
cross_links:
  - rel: supports
    target_object_id: writing_scale_work_history_detail_by_role_relevance
reference:
  source_title: "Writing Resumes & Cover Letters For Dummies, 2nd Australian and New Zealand Edition"
  author: Amanda McCarthy and Kate Southam
confidence: high
references: []
variants: []
---

# Write Specific Responsibility Statements

## Pattern Rule
**IF** a resume must describe what the candidate was employed to do
**THEN** name each selected responsibility with an accurate action verb plus the task, method, subject, or scope that makes the work concrete

## Do
- Choose a verb that identifies the actual work, such as negotiated, reconciled, diagnosed, scheduled, or trained, instead of relying on a generic activity label.
- Use present tense for continuing duties and past tense for completed or former duties.
- Add the object and useful boundary of the action: what was managed, for whom, through which process, or at what scale.
- Select responsibilities that illuminate the target role rather than reproducing the whole position description.
- Keep paid duties distinct from achievements so the reader can tell assigned scope from added value.

## Don't
- Treat an action verb by itself as evidence; managed or coordinated still leaves the reviewer asking what and how.
- Use vague connectors such as liaised with when a more exact verb can state the exchange, decision, or result.
- Mix current and past tense randomly within one role.
- Copy every routine duty until the important responsibilities disappear in a catalogue.

## Checklist
- Each statement answers what the candidate actually did.
- Verbs match the candidate's authority and do not inflate participation into ownership.
- Tense agrees with whether the responsibility is current or completed.
- At least the responsibilities central to the target role include concrete scope or context.
- No responsibility is being presented as an achievement without a distinct benefit or result.

## Notes
Responsibilities establish the candidate's entrusted scope. Their evidentiary value comes from precision: a short statement can distinguish advising from deciding, assisting from leading, or processing from redesigning. Verb lists are prompts, not finished copy; the noun, method, and boundary determine whether the claim can be understood.

---
object_id: writing_ap_finalize_and_submit_resume
object_type: ap
name: Finalize and Submit a Resume
library_path:
  - writing
  - career-documents
  - resumes
...
# Finalize and Submit a Resume

## Objective
Take a completed resume and its required application materials through targeting, content, language, presentation, proof, file, routing, and deadline gates until the exact package intended for the employer is verified and ready to send.
...
8. **Run the irreversible send check.** Compare the outgoing recipient, vacancy identifier, message, attachments, versions, formats, and deadline with the frozen contract. Stop only when every required item is present, every file opens as intended, the correct recipient and role are named, and the package can be submitted on time. If any check fails, correct it and repeat the affected downstream gates before sending.

## Notes
The order protects expensive final checks from being invalidated by a late targeting or content change. Proofreading a document whose evidence is still wrong creates confidence without readiness; testing a local file without transmitting it misses compatibility failures; checking attachments before the recipient and vacancy are fixed can route a polished package to the wrong destination. The protocol ends at verified readiness or completed submission, not at the feeling that the resume has been reviewed enough times.

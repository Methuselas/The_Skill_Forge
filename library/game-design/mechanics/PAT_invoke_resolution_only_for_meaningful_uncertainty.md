---
object_id: PAT_invoke_resolution_only_for_meaningful_uncertainty
object_type: pattern
name: Invoke Resolution Only for Meaningful Uncertainty
library_path:
- game-design
- mechanics
...
# Invoke Resolution Only for Meaningful Uncertainty

## Pattern Rule
**IF** an outcome is genuinely uncertain and the possible results materially change risk, cost, time, information, access, position, resources, or another meaningful game state
**THEN** invoke the appropriate resolution mechanic and let the result decide which consequential state follows
**ELSE** allow the action, summarize the inevitable result, or state the time or cost required without manufacturing a check.

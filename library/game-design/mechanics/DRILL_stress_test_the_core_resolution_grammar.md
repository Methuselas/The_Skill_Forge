...
## Practice Task
Run a mixed set of ordinary, pressured, unusual, and subsystem-like situations through the current core mechanics, then perform one incremental integration pass on a known-working resolution procedure. If a replacement core grammar is under consideration, run representative cases through both the current and replacement versions. Record which cases need no check, which fit the established grammar, which additions integrate cleanly, what the replacement actually improves or disrupts, and which cases genuinely require refactoring or specialized procedure.
...
12. If a replacement core grammar is proposed, resolve the same representative actions through both versions. Compare probability or difficulty transparency, meaningful decision points, procedure time, lookup or translation burden, affected dependencies, and compatibility with existing character or content data.
13. Remove any proposed check, mechanic, or procedural stage whose execution cost does not buy a meaningful difference in play.
...
- When a replacement grammar is being considered, at least one identical case was actually executed through both versions and the record compares transparency, decision value, procedure burden, and compatibility rather than merely stating a preference.
...
- Declaring a replacement grammar superior because it is newer, more unified, or mathematically elegant without executing equivalent cases and measuring what existing fluency or compatibility it displaces.

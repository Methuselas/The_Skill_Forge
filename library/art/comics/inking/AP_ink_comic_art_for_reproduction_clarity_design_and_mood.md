...
4. **Know the reproduction target continuously.** Apply `PAT_match_rendering_complexity_to_reproduction_process` at this decision. Use the actual output size/format as the criterion for detail and line separation. When fine information cannot survive reduction, simplify or translate it instead of making every small line darker.
5. **Establish the broad black-white design before texture.** Apply `PAT_design_whole_picture_as_interlocking_shape_pattern` at this decision. Resolve the main figure-ground, large black masses, and light/middle/dark organization. A page or panel does not pass this gate if broad clarity is weak; more feathering cannot rescue a failed large read.
6. **Resolve the light hierarchy and line system.** Apply `PAT_design_lighting_to_serve_subject_mood_and_visual_intent`, `PAT_concentrate_contrast_and_accents_at_focal_area`, `PAT_pair_directional_line_and_mass_marks_as_complementary_roles`, `PAT_bias_line_weight_toward_shade_and_underside_for_depth`, and `PAT_select_and_shape_contour_for_expressive_meaning` at this decision. Let the primary light/value logic govern contour emphasis, then coordinate contour character, line weight, black mass, feathering/hatching, and interior detail as one finishing system. Route local mark behavior to the accepted contour, line-weight, and value-translation owners.
...
8. **Interpret hatching and texture by function.** Apply `PAT_translate_value_into_mark_density_and_open_ground` at this decision. Decide whether a passage needs value graduation, softer black transition, form description, texture, or mood, then use line density/open ground accordingly. When that passage is being handled with brush and ink and needs a soft tone, broken texture, or darker mass, apply `PAT_control_brush_ink_tone_with_moisture_and_pressure` rather than treating moisture/load as an accidental variable. Texture remains subordinate to figure-ground and focal hierarchy.

...
`VAR_loomis_triangulate_portrait_identity_across_multiple_references` adds a likeness-specific synthesis pass: collect several views, identify recurring skull/face proportions and feature spacing that persist across source accidents, separate those stable traits from expression/camera/light artifacts, then rebuild the subject through one coherent target construction. This differs from the reference-normalization variant because the thing being reconciled is subject identity rather than camera and illumination.

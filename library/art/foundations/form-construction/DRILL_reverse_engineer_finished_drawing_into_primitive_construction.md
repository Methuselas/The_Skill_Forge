...
## Notes
Pair finished drawings with simplified sphere, cube, and cylinder analyses, then apply the same backward comparison to foreshortened figures or products. The durable exercise is not copying one exact primitive vocabulary; it is asking what solid construction must exist for the finished drawing to be possible. A camera may reduce to block-plus-cylinder structure, an angle grinder to a positioned cylinder with attached solids, and a wheelbarrow to a tub, wheel, and tube frame. Test not only primitive choice but also connections and whether a very basic version still preserves the object's nature.

...
## Notes
Contrast image types that receive prolonged attention with formats that must communicate quickly at a distance, and ask whether the idea reads in the available time, survives its medium, and can lose anything else without damage. Add a persistence test: an effect that works once may become stale or irritating when seen every day. Design for the actual viewing condition rather than for the drawing board alone. Cover and jacket work adds a competitive-context check because a design that succeeds against a neutral workspace can disappear among equally assertive neighbors. `VAR_loomis_reduce_glance_read_display_to_one_dominant_unit_and_minimal_message` preserves the extreme short-read case; repeated-view formats impose the opposite test, where a successful glance effect may become tiring over months. `VAR_bluth_make_brief_storyboard_images_read_at_a_glance` applies the same rule to boards whose principal action and message must survive almost immediately through clear silhouette, separation, and focal emphasis. `VAR_stanchfield_scale_animation_acting_to_final_viewing_distance` applies it to performance: test at actual character size and viewing distance, strengthen broad pose and action information when subtle acting disappears, and retain small acting when the shot supports it.

...
- variant_id: VAR_dodson_merge_same_value_shapes_across_object_boundaries
  ...
  difference_from_foundation: Uses connected major light or dark families as a two-dimensional scaffold across object boundaries;
    when both families form useful interlocking systems, preserve both and question isolated floating value islands unless
    they perform a necessary structural, spatial, or focal job.
  when_to_use: Use when simplifying or choosing a view reveals connected value families that can unify otherwise separate
    objects into one picture pattern.
  when_not_to_use: Do not force all values into one connected family or delete isolated accents that are necessary for structure,
    depth, or focal hierarchy.
...
- variant_id: VAR_dodson_reduce_scene_to_nonobject_shape_language
  ...
  difference_from_foundation: Progressively reduces source detail to reveal value mergers and hierarchy at several scales,
    then returns to full clarity for actual value/color judgment; the simplified view is evidence about grouping, not a literal
    sample of the softened appearance.
  when_to_use: Use mild reduction to reveal local mergers, stronger reduction for major value families, and extreme-but-readable
    reduction when the dominant light/dark split is uncertain.
  when_not_to_use: Do not paint the darkened or desaturated appearance created by the reduction itself, and do not delete
    small high-contrast information that remains structurally characteristic or focal.
  absorbed_from_object_id: none
...
`VAR_dodson_reduce_scene_to_nonobject_shape_language` separates two jobs: simplified viewing discovers grouping and hierarchy, while clear viewing supplies the actual value/color evidence. `VAR_dodson_merge_same_value_shapes_across_object_boundaries` treats connected major light and dark families as a compositional scaffold rather than as object-local shading.

...
## Notes
A storyboard "hook-up" is a shot transition that minimizes the audience's awareness of the camera change while the action itself continues. Preserve swing direction, running speed, or other active motion through the cut; even a small unexplained camera shift between near-duplicate views can read as an accidental jump. This Pattern owns the storyboard-stage decision of where to cut and what continuing action and camera state the incoming board must inherit. It differs from `PAT_match_action_state_across_shot_boundaries`, which governs the animator's downstream character/prop continuity after shot design is established.

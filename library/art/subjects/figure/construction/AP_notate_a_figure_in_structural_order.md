...
## Objective
Convert an accepted Stage 1 action framework into a coherent Stage 2 figure by resolving the torso, support system, limbs, and head in dependency order while preserving the original action, attachment logic, and spatial direction.
...
6. **Pass the typed limb-chain gate before anatomy can hide a mistake.** Invoke `AP_audit_articulated_limb_identity_and_joint_mechanics` at Stage 2 resolution. Confirm each represented limb still traces from its correct parent origin through the intended major joint sequence to the correct endpoint type, with anatomical/body-plan identity kept separate from screen position. Require major elbow/knee facing to be coherent enough that Stage 3 will not need to reverse a hinge or exchange a limb identity; do not demand finished joint surfaces at block resolution.
7. **Resolve the head and neck after the body action is stable.** Apply `PAT_resolve_head_and_neck_last_from_context` at this decision and `PAT_seat_neck_inside_upper_chest_opening` to keep the head/neck solution attached to the accepted torso. Keep the earlier head direction as a guide. When the Stage 2 head requires structural construction beyond the temporary framework oval/axis, delegate that bounded head decision to `AP_construct_figure_head_from_cranial_structure_to_living_character` at Stage 2 resolution, then reintegrate the returned cranial/facial block without changing the accepted body action or shoulder-girdle attachment.
8. **Pass the dependency rollback gate.** If a later limb or head solution can work only by silently moving the accepted torso, support, attachment, or major action, return to that earlier dependency and repair it. Do not absorb the contradiction downstream with contour or anatomy.
9. **Delegate severe Stage-2 foreshortening when necessary.** When apparent size, hidden paths, overlap, or advancing/receding member identity is the unresolved problem, invoke `AP_control_foreshortened_form_size_in_stage_two`, then reintegrate the returned construction into the parent action/support system.
10. **Pass the notation exit gate on a plain construction.** Read the figure without detail: the torso centerline governs the action; support/suspension/momentum is believable; limbs attach once; hands and feet are structurally placed; and the figure can advance without later stages inventing a missing connection.
11. **Advance without abandoning the dependency solution.** Stage 3 may make the construction more specific and Drawing Stage 4 may integrate final pencil line/value, but later work must preserve the Stage 2 action, support, attachments, and major spatial relations unless the workflow explicitly rolls back. Color and other medium finish remain downstream.

## Notes
Persistent invariants are **ACTION**, **SUPPORT**, **ATTACHMENT**, and **DEPENDENCY**. Later members may clarify the pose but may not silently rewrite the structural decisions they depend on.

The normal Stage 2 dependency order is torso → legs → arms → head because it makes support and attachment errors visible early. Case-specific exceptions may follow the actual mechanics of the pose—for example a hanging figure may need the supporting arms resolved earlier—but the exception must still honor dependency rather than convenience.

Step 1 uses symbols for position and direction; the three-dimensional masses begin in Stage 2. The centerline is carried forward as a governing constraint rather than added afterward to rationalize independently placed forms. Severe recession may still need the dedicated foreshortening/unity tools downstream; this AP should expose that need rather than hide it with anatomy or rendering.

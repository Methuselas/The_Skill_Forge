...
2. **Assemble a timed previsualization.** Put the boards or equivalent rough poses into story order and assign shot and beat durations. Add temporary dialogue, music, or sound when it materially affects the timing decision. When the finished piece has a fixed running length, time the complete sequence early: total the sections, compare them with the required duration, and rebalance, trim, omit, or invent only enough business to make the whole fit before expensive animation begins. If recorded dialogue carries an intentional performance whose pauses cannot be changed freely, treat that duration as committed temporal territory and deliberately allocate the remaining time among reactions, pantomime, transitions, establishing shots, action, and other non-dialogue business. Use `PAT_phrase_animation_timing_around_story_accents_and_action_beats` when the preparation, action, reaction, or accent timing is not yet readable.

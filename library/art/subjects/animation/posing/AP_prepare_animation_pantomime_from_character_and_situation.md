...
4. **Design one clear action or gesture at a time.** Use `PAT_design_animation_extreme_as_storytelling_pose` when a decisive pose must carry the beat. Keep the primary statement simple and coordinate the whole body around it. Let secondary actions support the current idea rather than compete with it.
...
6. **Give every movement a reason.** Use `PAT_track_force_continuity_through_action` when connected action loses directional or energetic continuity between poses. Ask whether each gesture clarifies who the character is, what they feel, what they want, or why they are acting. Remove motion that is merely decorative or generic.
...
10. **Run the pantomime gate.** Use `DRILL_test_animation_acting_as_pantomime_without_supporting_channels` to remove dialogue and supporting channels temporarily. Repair pose, gaze, weight, silhouette, or phrasing if the body no longer communicates the intended performance. Complete only when the intended acting change remains readable through the body without requiring the removed channels to explain it.

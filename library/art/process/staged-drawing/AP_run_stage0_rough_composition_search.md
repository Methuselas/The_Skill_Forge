...
2. **Choose the Stage 0 search form.** For an open composition, use `PAT_explore_stage0_with_thumbnail_set`; for a fixed direction, make one rough control thumbnail. Prefer four to six separate candidate image outputs when the host can return multiple images from one request; use one neutral thumbnail sheet only when separate outputs are unavailable. Use the single universal Stage 0 low-information ceiling and `PAT_calibrate_stage_information_density_against_precedent` to keep every result cheap enough to reject.
...
5. **Compile the productive image through the artifact-only handoff.** Use `AP_prepare_artifact_only_image_generation_handoff` and the Productive Image Contract below. Do not narrate the staged workflow immediately before generation, and do not add later-resolution visual requirements merely because the golden-truth pack contains them.
...
9. **Return the Stage 0 artifact and stop for ratification.** When Search is open, return roughly 4–6 genuinely different rough candidates, preferably as separate image outputs when the host supports them; otherwise use one neutral contact sheet. When the direction is already narrow, one rough control thumbnail may be enough. After selection, if the approved root exists only as a panel inside a contact sheet, preserve that panel as the semantic Stage 0 identity but do not assume independent native edit/reference access; any Stage 1 operation that requires exact continuity must pass the exact-predecessor accessibility gate before generation. In all cases, stop after the current Stage 0 artifact and await explicit user selection, revision, or rejection.
...
- **Artifact form:** prefer four to six separate loose composition sketches returned together when the host supports multiple image outputs. Otherwise use one simple contact sheet containing four to six independent thumbnails and nothing else. No title, captions, notes, legends, “chosen” panel, later-resolution panel, or presentation-board layout.
- **Preserve:** the user's subject/story requirements and stage-appropriate character identity constraints only. Do not solve later surface design merely because the references make it known.
- **Show:** strongly different camera/action/composition families using gestural figure shapes, silhouette, large environment masses, simple perspective/depth cues, foreground/midground/background relationships, and at most a few broad value families.
- **Diversity gate:** no two alternatives should merely be cosmetic variations of the same shot. Across the set, materially vary several of: action family, camera height/side/roll/distance, torso orientation, support/contact surface, primary diagonal, subject scale/crop, foreground relationship, silhouette family, and depth path. If the user has rejected a family, do not reuse its defining descriptor bundle unless they explicitly reopen it.
- **Action gate:** when a semantic action quality is required, it must survive as gesture and silhouette alone. A candidate that becomes “dynamic” only after adding a gun, neon, motion effects, dramatic lighting, or costume detail fails.
...
- **Withhold:** resolved facial features, hair strands, costume seams/emblems, detailed armor construction, polished anatomy, finished hands or weapons, surface materials, texture, glow/reflection effects, atmospheric rendering, detailed signage, polished lighting, and final environment description.
- **Global ceiling:** every candidate remains inexpensive and disposable. Sketchy linework over a developed illustration still exceeds the ceiling.
- **Stop:** when the alternatives are distinct enough to choose among on composition/action/camera grounds alone.

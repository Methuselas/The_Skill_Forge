...
reference:
  source_title: Guided Observatory Composition Search and Rosetta Backcast
  author: MaDin + GPT
confidence: high
references:
...
- variant_id: VAR_mogilevtsev_preview_final_impression_with_integrated_study
  ...
  variant_basis: method_sequence
  difference_from_foundation: 'Adds the committed control-study route for a fixed long, large, complex, or expensive final:
    make one small preliminary study in a format corresponding to the intended final work, and use it to preview the finished
  ...
  when_to_use: Use when the model or assignment is fixed and a long, large, complex, or expensive drawing needs a compact
    ...
    when later observation drifts from the chosen action. Use reduced-scale or distance checks during execution when the work
    risks becoming coherent only at close local inspection.
...
## Don't
- Do not produce four cosmetic variants of the same camera, action family, support relationship, and silhouette when the point of Stage 0 is Search. Different rendering or one moved limb does not create a new branch.
...
- Any explicitly required action quality (for example acrobatic or dynamic) reads from gesture/silhouette without relying on finish.

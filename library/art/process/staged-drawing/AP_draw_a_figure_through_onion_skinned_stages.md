...
## Steps / Flow
1. **Resolve delivery mode and continuity first.** Apply the scene-wide predecessor invariant from `PAT_develop_scene_through_registered_successors`. If the user did not request visible drawing stages, internalize Stages 0–3 and surface only the requested Drawing Stage 4 finished pencils; requests for Ink/Color/Paint require the applicable downstream workflow rather than redefining Drawing Stage 4. If visible staged production is requested, use `AP_gate_staged_visual_work_by_approval` and conversational `S#-r#` labels for actual generated artifacts.
...
9. **Delegate and audit high-risk figure sub-actions before approval.** Prominent functional hands may invoke `AP_construct_hand_from_function_contact_and_articulated_form`; severe foreshortening may invoke `AP_unify_a_foreshortened_figure_in_deep_space`. For materially represented articulated limbs, invoke `AP_audit_articulated_limb_identity_and_joint_mechanics` at the current resolution so crossed, twisted, inverted, or foreshortened chains cannot pass merely because they remain connected. Recheck every returned local result against the approved whole.
10. **Inspect and gate Stage 3.** Approve only when the serious drawing works at full size and still reduces to the approved Stage 0 proposition. Rendering quality never excuses scene drift, unauthorized inventory, endpoint substitution, or mechanically impossible visible limb articulation.
...
12. **Final continuity and limb-mechanics check.** Compare the final against both anchors, then re-run `AP_audit_articulated_limb_identity_and_joint_mechanics` on the finished figure because finishing or regeneration can disturb an otherwise accepted chain. Repair local Stage 4 defects as Stage 4 revisions; route earlier-property failures back to the owning construction stage.

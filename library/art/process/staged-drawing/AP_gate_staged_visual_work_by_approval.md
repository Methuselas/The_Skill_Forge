...
12. **Generate one image per native call, with Stage 0 cardinality owned only by the controller.** Open composition Search produces **four to six individual candidate images total for the search cycle**, normally four, by repeating a singular native image invocation. **One native image call = one Stage 0 candidate.** Never use a host multi-output option to satisfy the Stage 0 budget, never place the global count or candidate index in the image-facing contract, never generate a contact sheet, production sheet, grid, or multi-panel candidate canvas, and never request another full four-to-six cycle because one candidate failed. Every later artifact is likewise one full-frame image. Never turn the workflow itself into a visual deliverable unless the user explicitly asks for a separate presentation artifact outside productive lineage.
13. **Inspect the returned artifact against the current AP.** Check stage purity and picture identity immediately. At Stage 0, also reject any image that contains more than one composition candidate, presentation-board framing, in-image candidate ranking/selection, or later-stage rendering; an invalid candidate does not count toward the valid four-to-six total. If the host permits private candidate inspection before surfacing, use it; if the host displays generation immediately, mark a violating artifact invalid, do not approve/anchor it, and retry only the missing candidate slot or rollback. Never pretend an invalid image was legitimate merely because it is attractive.
...
15. **Gate and stop only after the complete visible Stage 0 cycle.** For open Stage 0 Search, issue the bounded sequence of singular image calls, surface only the resulting individual rough propositions, then stop for user selection/revision/rejection; do not recommend or mark a winner unless the user separately asks for critique. For later stages, stop after the one current artifact. Do not place the Stage 0 batch count, candidate numbering, Search terminology, or approval instructions in image-adjacent productive text merely because a host restricts post-image messaging; omit that narration rather than contaminating the native task. One approval authorizes at most one successor artifact.

...
5. **Check environmental consistency.** Compare objects under the same light. Make sure illumination, reflected surroundings, and atmospheric influence affect them as parts of one scene rather than as isolated color swatches.
6. **Use muted support deliberately.** Reduce unnecessary chroma in broad or subordinate areas when stronger focal color needs a quieter foil.
7. **Restore color selectively if the whole picture is genuinely over-greyed.** Increase chroma only in the passages that need it, preserving hierarchy and the established value/light structure.
8. **Judge the result small.** Step back or reduce the image so the large color relationships can be compared without local detail distracting from the diagnosis.

---
object_id: DRILL_copy_then_emulate_master_mark_language
object_type: drill
name: Copy Then Emulate a Master's Mark Language
library_path:
- art
- drawing
- foundations
- mark-making
stage_binding: 3 rough
lane_fit: teach
foundation_role: foundation
routing_class: general
specialization_axis: none
foundation_object_id: none
tags:
- master_study
- mark_making
- style_study
- emulation
cross_links:
- rel: teaches
  target_object_id: PAT_alternate_free_search_and_controlled_refinement
reference:
  source_title: Keys to Drawing
  author: Bert Dodson
confidence: high
target_skill: recognizing how a particular artist's stroke length, pressure, pace, connection, and rhythm create a transferable mark-making system
references: []
variants: []
---

# Copy Then Emulate a Master's Mark Language

## Practice Task
Study one drawing by copying its mark behavior, then draw a different subject from life or reference while borrowing the same mark-making logic.

## Target Skill
Recognizing how a particular artist's stroke length, pressure, pace, connection, and rhythm create a transferable mark-making system.

## Setup
Choose a master drawing with clearly legible marks and use a reasonably similar drawing medium for the copy. Prepare a new subject for the emulation pass.

## Instructions
1. Copy a small passage while attending to how the marks were likely made: length, direction, connection, pressure change, speed, edge, and rhythm.
2. Compare the physical behavior of your marks with the source rather than judging only whether the depicted object looks similar.
3. Put the master drawing aside or at a secondary reference position and draw a new subject using the borrowed handwriting.
4. Note which mark behaviors transfer and which collapse when they are no longer tied to the copied shapes.
5. Repeat later with a markedly different artist rather than settling permanently into one borrowed manner.

## Success Check
The second drawing applies recognizable mark-making behavior to new forms without tracing or repeating the original composition.

## Common Failures
- Copying subject contours while ignoring the physical character of the strokes.
- Treating emulation as permanent style adoption.
- Choosing a different medium whose behavior makes the target mark language impossible to test.

## Notes
Dodson separates copying from emulating: copying gives a felt understanding of another artist's handwriting, while emulating tests whether that understanding can survive on a new subject.

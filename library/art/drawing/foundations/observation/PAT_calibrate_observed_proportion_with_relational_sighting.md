...
- variant_id: VAR_guptill_register_drawing_against_observed_source
  ...
  variant_basis: method_sequence
  difference_from_foundation: 'Adds a direct registration check after an observational block-in: align the drawing with the same-view source, inspect where major contours and proportional landmarks fail to coincide, correct the earliest large mismatch, then recheck before local contour refinement.'
  when_to_use: Use when individually plausible contour fragments have accumulated into a drawing whose overall proportions no longer match the observed source.
  when_not_to_use: Do not use registration as a substitute for observation or as a license to trace every contour; it is a diagnostic for locating large relational drift.
  absorbed_from_object_id: none
---

...
- variant_id: VAR_guptill_match_final_line_character_to_shape_and_texture
  ...
  difference_from_foundation: 'After construction and proportion are secure, vary final contour character with the represented
    ...
    locally graded where the subject calls for it. For worn or irregular constructed surfaces, controlled breaks, slight edge
    unevenness, chipped corners, dents, and altered rhythm can carry age or deterioration while enough underlying geometry
    remains intact to keep the object visibly constructed.'
  when_to_use: Use in line-led rendering when a uniform contour makes unlike shapes or materials feel mechanically identical,
    or when controlled irregularity is needed to distinguish a worn built surface from a pristine one.
  when_not_to_use: Do not vary line character randomly, let surface suggestion override the structural contour that still
    has to read, or use age-related irregularity so broadly that construction begins to look inaccurate rather than worn.
  absorbed_from_object_id: none
- variant_id: VAR_martin_match_contour_character_to_source_pencils
  variant_name: Match Contour Character to Source Pencils
  variant_basis: context
  difference_from_foundation: When finishing an existing drawing, chooses a contour language that complements the source pencils
    instead of automatically imposing one habitual line; candidate systems may differ in where thickness peaks within a stroke,
    smooth/cursive versus terse/angular character, variable versus deliberate dead-weight contour, and the contrast between
    exterior contour and interior detail. Recurring idiosyncratic marks are treated as possible parts of the source artist's
    visual grammar rather than normalized automatically when their literal anatomy or material meaning is unclear.
  when_to_use: Use when the underlying construction is already supplied and several plausible contour languages could finish
    it with materially different softness, hardness, energy, or graphic character, especially when the source has a distinctive
    or initially unfamiliar mark vocabulary.
  when_not_to_use: Do not turn source compatibility into passive tracing, preserve a treatment that conflicts with the intended
    light, form, texture, or reproduction need, or "correct" an unusual recurring mark until you have checked whether it carries
    a stable expressive role in the source language.
  absorbed_from_object_id: none
- variant_id: VAR_martin_keep_cartoon_contours_bold_but_fluid
  variant_name: Keep Cartoon Contours Bold but Fluid
  variant_basis: context
  difference_from_foundation: In lively cartoon line art, establishes a strong exterior/holding-contour role against lighter
    interior description while preserving variable-width energy, selective contour dropout, and stylistic continuity between
    figures and backgrounds; boldness is treated as a hierarchy requirement rather than a uniformly heavy dead-weight band.
  when_to_use: Use when simplified or cartoon-styled forms need a silhouette strong enough to survive reduction without losing
    the spontaneous, open, or elastic line character that gives the drawing life.

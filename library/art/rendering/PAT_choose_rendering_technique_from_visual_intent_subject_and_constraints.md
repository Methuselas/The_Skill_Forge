...
**THEN** choose the overall handling system from the intended visual statement, subject scale/pattern character, required edge and surface behavior, available time, and real medium/tool/support constraints rather than from habitual style
...
- When the support materially changes handling, treat medium, tool, and support as one working system; account for surface tooth, absorbency or water response, ground tone, and dimensional stability under the intended fluid load rather than assuming the named medium behaves the same everywhere.
- For fluid techniques, test whether the support remains acceptably stable at the intended saturation; if it buckles, warps, or otherwise changes enough to disrupt the handling, change the support/preparation or change the technique rather than relying on nominal paper weight alone.
- Calibrate an unfamiliar medium/tool/support combination with a small behavior test before depending on an assumed intensity, edge, spread, or coverage response.
...
- Time, reference, medium, and support constraints support rather than contradict the intended finish.
- The chosen medium/tool/support combination has been calibrated when its behavior is not already dependable.
...
Treat named media and techniques as behavior bundles rather than hard visual identities. When the support changes tooth, absorbency, water movement, dimensional stability under fluid load, or the apparent value/chroma of marks, include that support in the behavior bundle instead of treating it as a neutral substrate. If the available medium can cleanly produce the transparent, dry, broken, graphic, smooth, scraped, or other behavior the picture needs, choose by the required result rather than by what the medium is conventionally supposed to look like. Do not imitate another medium merely as novelty, and do not force emulation when another available method solves the visual problem more simply or reliably.

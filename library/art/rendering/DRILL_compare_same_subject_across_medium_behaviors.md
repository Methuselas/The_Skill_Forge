...
- variant_id: VAR_schmid_remove_familiar_tool_to_force_alternate_handling_language
  ...
  difference_from_foundation: Keeps the subject and visual goal stable while temporarily forbidding the habitual primary tool
    or handling route, forcing another medium behavior to solve the same edge, mass, thickness, stroke, and simplification
    jobs.
  when_to_use: Use when a comfortable tool is making handling automatic and the model needs to discover alternate visual routes
    rather than merely reproduce a familiar surface.
  when_not_to_use: Do not keep the restriction when the tool is genuinely required by the final assignment; the constraint
    is a discovery rep, not a doctrine of tool avoidance.

...
- Use the same shape to bracket a call rather than to observe it. Where access must be surrounded by work on both sides — a lock taken and released, a transaction opened and committed, a use counted in and out — returning a temporary whose constructor does the first half and whose destructor does the second brackets whatever the caller does in between, without the caller writing anything. Arrow dereference is applied repeatedly until it reaches a plain pointer, so a proxy returned from it keeps ordinary member-call syntax at the call site.
...
The bracketing use is worth separating from the observing one, because the risk is different. A proxy that exists to observe what happens next is read-only in effect and cheap to get wrong safely; a proxy that holds a lock for its own lifetime hands the caller a duration they did not ask for and cannot see. Binding it to a temporary makes that duration exactly one expression, which is the point — but a caller who binds the proxy to a named variable instead has extended it, so the bracket is only as tight as the shortest lifetime the type permits.

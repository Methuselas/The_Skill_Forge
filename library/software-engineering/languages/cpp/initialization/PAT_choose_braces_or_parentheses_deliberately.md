...
The vexing parse deserves more than the one clause it gets above, because the empty-parentheses case is the mild version and the general rule is what actually bites. Anything that can be parsed as a function declaration will be, and a parenthesized argument that is itself a type name with parentheses reads as a parameter declaration rather than as a value. So an object constructed from two parenthesized temporaries declares a function returning that object type — with a first parameter named by what you meant as a variable, and a second parameter of pointer-to-function type. Unlike the empty-parentheses case there is no warning and nothing to notice: it compiles, and at run time the object simply never exists, so the code that was meant to fill it does nothing.

Braces defeat the general form as well as the empty one, which is the strongest single argument in their column. Where braces are unavailable or unwanted, the other escape is to give the arguments names — declare each temporary as its own named variable and pass those. That is unfashionable and verbose, and it is unambiguous to compilers and readers alike, which on this particular construct is worth more than the brevity.

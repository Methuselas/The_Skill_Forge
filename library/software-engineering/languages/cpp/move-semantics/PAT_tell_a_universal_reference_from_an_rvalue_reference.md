...
- Recognize the same form in a generic lambda, where a parameter declared `auto&&` is a universal reference like any other. Forwarding it needs the type, and the parameter has no name you can write as a type — so the conditional cast takes `decltype` of the parameter, which yields exactly the lvalue or rvalue reference the collapse rules produced.

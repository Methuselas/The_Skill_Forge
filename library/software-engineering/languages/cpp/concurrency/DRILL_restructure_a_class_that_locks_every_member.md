...
## Instructions
1. Find every path where a public function reaches another public function on the same object — directly, through a stored callable, through a base-class pointer, or through a virtual dispatch that lands back on this object. Say what happens at each with an ordinary mutex, and what happens with a recursive one.
...
3. Check the accessibility of every function that still locks, and every function that no longer does, against the rule — not against what the code happened to need. Name every exemption you allow — constructors, destructors, and members touching only state that is immutable after construction — with the reason it is one.
...
7. Find any place the class calls out to something it does not control while holding either mutex — a callback, a comparator, a virtual call — and restructure so the call happens outside the guarded region. Then list every remaining call in a guarded region that reaches code the class does not own — an allocation, a formatting or logging call, a standard-library entry point — and either relocate it or keep it deliberately with the reason it cannot participate in a cycle here.

...
The one-job rule extends past routines to loops, but the detector does not travel with it: a loop carrying two jobs per pass reads aloud perfectly well, because the reason for merging was speed rather than clarity. That case needs its own procedure and has its own card, `PAT_keep_one_job_per_loop`. Read it whenever the argument in front of you is about traversal cost rather than about how the code sounds.

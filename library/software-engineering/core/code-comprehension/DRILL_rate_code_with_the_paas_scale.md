...
What makes the recommendation credible is the alternative it was compared against. The instrumented options are worth knowing: eye tracking, where blink rate falls and pupil size rises with load, on the hypothesis that the brain is maximising visual intake on a hard task; skin temperature and sweat; EEG; and fNIRS, which infers load from oxygenated haemoglobin via infrared light. Nakagawa's fNIRS study found greater oxygenated blood flow on a deliberately complicated version of a C algorithm for 8 of 10 participants, with functionality held constant. All of it correlates with the nine-point question, which is why the drill uses the question.

The addition this drill makes to the original exercise is the repetition across several pieces of code, because a single rating tells you about one file while a run of them tells you about yourself.

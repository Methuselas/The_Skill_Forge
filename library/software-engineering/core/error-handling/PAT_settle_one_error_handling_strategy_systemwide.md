...
- Fix which layer validates. Either each class validates its own inputs, or a designated group of classes validates the system's data and everything inside that boundary may assume clean data. What breaks systems is neither policy — it is having both beliefs held at once, whether in different modules or inside one. A single class that checks its accessors' arguments and not its constructor's holds both, and the inconsistency is no more defensible for being small; it just costs less to fix while it still is.
...
Where the policy admits exceptions, one boundary belongs in it explicitly: what counts as exceptional enough to throw for. Signalling by exception is not a cheap alternative spelling of returning a value — leaving a function by throwing can run three orders of magnitude slower than returning from it, and a language's support for exceptions is generally paid for in program size and speed whether or not any are ever thrown. That cost is irrelevant at the rate genuinely exceptional events occur and decisive at the rate ordinary ones do, which is why the mechanism should not be carrying routine outcomes such as reaching the end of a traversal or terminating a loop. Fixing the boundary at policy time is the cheap moment; discovering it from a profile after the mechanism has spread through the codebase is not.

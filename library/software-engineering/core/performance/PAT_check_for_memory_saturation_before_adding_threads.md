...
A worked instance is worth having because it shows the ceiling arriving in code that has been written correctly. Summing a large array across four threads, with each thread accumulating into its own local variable so that nothing at all is shared, came out about twice as fast as the single-threaded version rather than the fourfold its author expected — and the processor utilisation showed the four cores were not saturated. The limit was the rate at which memory could deliver the data. That is the signature to recognise: threads added, sharing eliminated, speedup well short of linear, and cores idle rather than busy.

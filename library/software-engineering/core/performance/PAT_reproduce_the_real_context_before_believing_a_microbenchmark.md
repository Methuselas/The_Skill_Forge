...
## Do
- Fix the compilation context first, because it is by far the largest difference between a benchmark and a program. Compile the code under test in its own unit with whole-program optimization off, so the compiler cannot inline it into the fixture and specialize it for arguments the real caller never passes. That single precaution is what brought a set of substring-comparison timings back into agreement with the whole-program profile.
...
## Checklist
- Is the code under test in its own compilation unit, with cross-unit optimization off?

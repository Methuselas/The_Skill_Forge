...
## Notes
Two illustrations carry this more directly than prose. The first puts a single labeled box beside a vertical stack of indexed boxes, so the extension from variable to array is visible as literally the same object repeated. The second sets two hand-drawn function machines side by side — on the left `print_square(5)` with a figure carrying a 5 in, on the right `square(5)` with a second figure carrying 25 back out — which is what makes "input parameters only" versus "input and output parameters" an extension rather than a replacement.

The conflicting case is the variable as box against the variable as name tag or sticker. The box implies a container that could hold several things, "like a box can hold multiple coins or candies," and a sticker cannot be placed on more than one thing, so the second model rules out a misconception the first invites. This pairs with the finding from Hermans's own NEMO study that the two framings produce measurably different errors, which is developed separately under metaphor selection.

...
## Do
- Measure when the argument is worth settling. The gap is the count of lines between one reference and the next; the life is the last referencing line minus the first, plus one. In the source's worked contrast, three variables initialized in a block at the top of a routine score lives of 27, 67 and 67 — an average of 54 — and moving each initialization down to just above the loop that uses it gives 4, 8 and 8, for an average of 7.

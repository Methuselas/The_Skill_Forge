...
- Recognise the third kind of change, which preserves behaviour like a refactoring but is its opposite in intent: a performance tuning deliberately *degrades* internal structure to buy speed. Both are behaviour-preserving, so both verify the same way, and calling a tuning a cleanup is how a codebase acquires damage that nobody signed off on.
- Apply the razor that follows: if a change costs no structure, it is not an optimization at all — it is ordinary good practice you should already be doing, and it needs no performance justification to earn its place.
...
Setting refactorings beside performance tunings is what makes the taxonomy complete, because the two look cosmetically identical and are opposite in what they do to the code. McConnell's name for tunings is **anti-refactorings**: far from improving internal structure, they degrade it in exchange for speed. His argument that this is true *by definition* is the useful part — if a change did not degrade the internal structure, nobody would classify it as an optimization, because everyone would simply use it by default as standard practice. So the two share a verification method and nothing else, and a change log that files them together loses the record of what was spent.

The razor cuts in a direction worth using, including against published technique lists. Several entries in the standard code-tuning catalogues fail it: hoisting a loop-invariant expression into a named variable outside the loop, or assigning a repeated subexpression to a well-named variable, both *improve* readability while possibly improving speed. By the definition above those were never optimizations, which means they need no measurement to justify and no tradeoff to defend — they are simply how the code should have been written. Sorting a proposed change this way first tells you whether you are about to spend structure or not, and that determines everything about how much evidence the change owes you.

The taxonomy has been argued against from the performance side, and the objection is worth recording because it looks stronger than it is. The claim is that speeding a routine up while leaving its inputs and outputs alone is plainly behaviour-preserving, so it belongs under the same heading as any other restructure. The reply is the razor above rather than a definitional preference: where the faster version costs no structure, nobody would have called it an optimization in the first place, and it needed no justification to be written that way. What remains under the objection is the case where speed was bought with structure, which is the anti-refactoring, and even those making the argument tend to concede that the clever, narrowly-scoped performance patch degrades faster than the code around it and is not the sustainable kind. So the disagreement is largely about which word to use, the sorting question the razor answers is unaffected, and the practical consequence — knowing whether this change is spending structure — is the same either way. Treat the boundary as settled.

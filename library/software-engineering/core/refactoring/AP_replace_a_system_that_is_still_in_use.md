...
1. **Establish that it is obsolete rather than old, and that a replacement covers the ground.** Age is not evidence. Name what is wrong that is not the date, and confirm the replacement handles every existing use rather than every use you have looked at. That distinction is `PAT_tell_obsolete_from_merely_old`.
...
2. **Work out how it got this way before deciding what to build.** Whether the demands moved or corners were cut determines what the replacement must preserve, and the constraints the original was quietly handling are the things a clean rewrite drops silently and rediscovers expensively. `PAT_diagnose_why_the_code_degraded_before_changing_it` owns the diagnosis, and `PAT_look_for_the_evidence_outside_the_code` owns where else to look for it.

3. **Baseline what you intend to improve, before touching anything.** A successful replacement is invisible in the metrics you already watch, so choose measures that capture the specific problem and record them now. Evidence outside the code — incident histories, question threads, change frequency — is where the cost of the current state is actually visible. `PAT_concentrate_effort_where_defects_concentrate` owns where the baseline is worth taking.

4. **Divide the work into milestones that are each a defensible place to stop.** Not steps sized to the calendar: states a stranger could read, extend, and ship from. Priorities move and people leave, and the division that survives suspension is the one designed expecting it. `PAT_make_every_milestone_a_place_you_could_stop` owns what makes a milestone defensible.
...
5. **Freeze new uses of the old path immediately.** Enforce it on changed code only, so the count of remaining callers can fall without anyone being asked to fix code they did not come to fix. Until the denominator stops growing, the migration has no end. `PAT_enforce_a_new_rule_only_on_new_code` owns the freeze, and `PAT_a_warning_migrates_nobody` owns why a warning is not one.
...
8. **Ramp the new answer outward in order of increasing risk**, scaling the care to what each step can reach rather than to how large it looks. Comparison coverage is proportional to traffic, so rarely-executed paths were never certified by step 7 and will be met for the first time here. Sequence the ramp so the first encounter is survivable. `PAT_judge_change_risk_by_what_it_can_break` owns the ordering.
...
10. **Remove the scaffolding, then the original.** The switch, the wrapper, the duplicated tests, the notes left for readers, and finally the superseded implementation itself. Downstream consumers you do not control come off it before it can be dropped. `PAT_remove_the_scaffolding_a_migration_leaves` owns the teardown, and `PAT_plan_the_removal_while_you_build` owns why it was survivable.

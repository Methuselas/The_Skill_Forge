...
## Notes
Figure 10.4 shows the progression more sharply than the prose does, using zero-based list indexing across three thought bubbles. In the cognitive phase the thought is a full sentence of reasoning — start at 0, so element 3 is the fourth one. In the associative phase it has collapsed to a rule — just always subtract 1. In the autonomous phase there is no verbal thought at all, only the list and `list[4]` with an arrow to the element. Verbal reasoning, then a rule, then direct perception.

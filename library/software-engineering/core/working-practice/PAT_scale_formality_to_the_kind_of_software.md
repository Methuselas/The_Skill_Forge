...
- Don't keep adding detail to a specification once the returns have gone flat. Past a point the curve turns negative: no document captures every nuance, users rarely know exactly what they need until they see something running, and a design that leaves the implementer no room removes the options that only become visible while coding — the "I could add this for almost nothing given how I built that" discoveries that a fully prescribed design has already ruled out.
- Don't stack specifications on specifications with no implementation or prototype underneath. Without something running to push back, it is entirely possible to specify a system that cannot be built, and nothing in the documents will say so.
...
- Is anything you publish being consumed as an interface by people you will never talk to?
- On the specification, is the next paragraph still reducing ambiguity, or just adding pages?
...
Failure cost sets the level, but it does not set a ceiling, and the specification control is where the missing ceiling does the most damage. Detail keeps looking productive long after it has stopped reducing ambiguity, partly because producing it feels like the work and partly because nobody can point at the line where it stopped paying. Three separate things cap it. No document captures every nuance of a system, and users generally do not know precisely what they need until they can use something — so the change requests arrive after the running system regardless of how thorough the document was. Natural language is genuinely not up to describing some procedures; the standard demonstration is trying to write instructions for tying a shoelace, which almost nobody completes and everybody can already do. And a specification detailed enough to leave no interpretation removes the discoveries that only happen during implementation, where the person writing the code can see that a small change to their approach would deliver most of the same result for half the work, or that what they just built admits an extra capability for nothing.

There is one axis where the failure-cost rule under-prescribes, and it is worth carrying separately: anything consumed as an interface by people you will not be talking to. Libraries, published APIs, and anything whose entire output is a set of calls somebody else writes against deserve careful specification whatever the failure cost of the system containing them, because the usual compensating mechanism — asking the author — is not available to the people who need it. That is a different argument from the safety one, and it lands on projects that would otherwise sit comfortably in the informal column.

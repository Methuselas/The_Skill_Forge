...
- Say which of four things you are building before you estimate, because the cost separates by roughly nine to one. A *program* is used by whoever wrote it. A *product* is used by strangers in environments its author never saw, so it is tested hard, documented, and maintainable by others — about three times the cost. A *system* is several programs that must interface and integrate — also about three times. A *system product* is both, at about nine times a bare program.
- Never estimate a piece of work from your own construction experience alone. Writing 2K lines of code does not take as long as producing a 2K-line program: an estimate built from the coding takes about 65 percent of the real time at that size, and about 50 percent at 32K, because the non-construction share grows as the thing gets bigger.
...
That range is a function of size, and it moves in one direction. Construction takes as much as 65 percent of development time on a small project, around 50 percent on a medium one, and a steadily smaller share as projects grow. The mechanism is that construction work scales roughly linearly with size while nearly everything else — communication, planning, requirements, interface specification, architecture, integration, defect removal, system testing, document production — scales faster. Ten times the size buys about twenty-five times the construction effort but forty times the architecture and system testing.

The estimation consequence is the practical one, and the two errors compound. Estimating from your own coding experience misses the non-construction share, which costs you about 50 percent more time than you predicted at 2K lines and about 100 percent more at 32K — and that error grows with size precisely because construction's share is shrinking. Layer on a failure to notice that the request is for a product rather than a program, or a system rather than either, and the underestimate can approach a factor of ten. Neither mistake is about being optimistic; both are about scoping the work wrongly before any estimating begins, which is what this card is for.

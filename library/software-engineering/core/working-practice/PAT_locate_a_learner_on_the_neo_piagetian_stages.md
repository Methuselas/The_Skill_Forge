...
Take all four applied to a three-line variable swap, and the progression is legible in what each programmer produces. The sensorimotor and preoperational programmers both say they do not know what it does; the difference is that the preoperational one has a trace table. The concrete operational programmer narrates the mechanism — z becomes y, y becomes x, x becomes z. The formal operational one states the meaning: the values of x and y are swapped.

The practical payoff is negative as much as positive. Three common interventions — general principles, diagrams, and unguided exploration — each fail at a specific stage, and knowing which is what stops a well-intentioned onboarder from adding load while believing they are removing it.

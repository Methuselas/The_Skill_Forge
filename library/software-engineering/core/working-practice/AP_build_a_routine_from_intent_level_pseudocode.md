...
Two limits worth stating. Applying this to trivial routines is waste, and the accessor case is the clearest example. And the flow is drawn as an ordered sequence only because something has to be drawn first — the standard diagrams put back-edges on every stage and caption them as not necessarily happening in any particular order, so treat the numbering as a default route rather than a required one.

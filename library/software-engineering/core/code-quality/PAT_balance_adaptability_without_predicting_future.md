...
## Notes
Long presents two extreme scenarios — exhaustively engineering for predicted change versus ignoring change entirely — and shows both lose to a competitor, one by shipping a year late and one by needing repeated three-month rewrites. The durable lesson is that adaptability is achievable without prophecy: general techniques keep code flexible without committing to guesses about which change will actually arrive.

...
Figure 9.2 makes the danger concrete in a way the prose does not. The two functions are identical apart from the else branch, where `foo` returns `j++` and `goo` returns `j+2` — a one-token divergence inside otherwise duplicated code, labelled Product A and Product B. That is precisely the shape mis-chunking hides, and Hermans notes it may take several exposures before you register that they differ at all, since the resulting misconception behaves like any other.

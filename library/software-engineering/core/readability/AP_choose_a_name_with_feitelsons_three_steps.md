...
## Steps / Flow
1. **Select the concepts to include.** This is domain-specific and Feitelson considers it the most important decision in naming. The governing question is the name's intent — what information the object holds and what it is used for. Two practical prompts: if you feel the need to write a comment explaining the name, the wording of that comment probably belongs *in* the name; and if a comment already sits next to a name in code you are reading, the same applies.

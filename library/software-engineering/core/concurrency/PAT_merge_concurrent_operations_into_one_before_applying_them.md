...
- Use the same staged structure to *distribute* as well as to combine, and take the platform's version of both when it offers one. Sending one value to every participant as a loop of individual sends is linear in the participants, because only the originator ever transmits; having each recipient relay onward as soon as it has the data makes it logarithmic. That is a better reason to prefer a provided collective operation than the usual one — not that it is tested, but that it has an asymptotically better implementation available to it that a caller writing a loop cannot express.
- Note that deferring the individual sends does not rescue the loop. Making each transmission asynchronous frees the originator but does not change when the last participant has the data, and it is the completion of the whole operation that the computation is waiting on.

...
## Instructions
1. For each design, state how many threads must agree on a single outcome. Distinguish that from how many threads merely touch the structure, which is a different and usually larger number. The answer may be none: a structure whose operations commute requires no agreement at all, however many threads use it, and that is a verdict rather than a dodge.

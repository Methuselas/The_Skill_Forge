...
- Pick deliberately among the three ways to make a compound sequence correct, because they differ in who carries the obligation. The caller can acquire before the first step and hold until after the last, which works and relies on every caller remembering. The owner can expose one operation that acquires, performs the whole sequence, and releases, which moves the obligation to one place. Or where the owner cannot be changed, an intermediary can sit in front of it and do the acquiring, which is the same move made from outside.
...
Of the three ways to fix a compound sequence, the middle one is the default and the first is the one that decays. Asking callers to acquire before they begin is correct on the day it is written and depends on a rule living outside the code that needs it — the next caller is written by someone reading the interface, which does not mention the obligation, and the omission produces no failure until the interleaving that exposes it. Exposing a single operation that does the whole sequence takes the rule away from the callers and makes it a property of the thing itself. The intermediary exists for the case where the owner is not yours to change, and it is worth recognising as the same fix rather than a different one, because it inherits both the benefit and the requirement that every path actually go through it.

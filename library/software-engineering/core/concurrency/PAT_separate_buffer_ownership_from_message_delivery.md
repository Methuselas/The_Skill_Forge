...
- Look for a second engine to overlap with, not just a caller to free. Where the transfer is performed by hardware independent of the processor doing the work — a direct-memory engine beside a compute unit is the clearest case — a design that transfers everything, then computes, then transfers back has serialized two units that could have run at once. Splitting the data into chunks and pipelining them, so one chunk transfers while the previous computes, converts the transfer time from an addition into an overlap. This is the same overlap argument as above with a second resource rather than a second thread doing the waiting.

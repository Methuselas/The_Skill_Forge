...
2. Split as finely as the algorithm allows — one piece per independent unit of work, which for an in-place sweep is not one piece per cell. State the piece count as a formula in the problem's own dimensions, or as the rule that generates the pieces where no formula exists. The count should be a property of the problem rather than of the machine, and should exceed the execution units by at least an order of magnitude; if it came out close to the unit count, you split to the machine rather than to the algorithm and must go back.
3. For every piece, write down what it needs from its neighbours and how much. Total the crossings and the number of transfers in a declared unit, showing the arithmetic that produced them.
4. Merge pieces into groups so the expensive crossings fall inside a group, aiming for a group count one to two orders of magnitude above the execution units so that placement still has slack to work with. Do this twice — once grouping along one dimension, once into rectangular tiles — and total the crossings and the number of transfers for each, in the same unit and again showing the arithmetic. Keep the two comparable: same group count, same unit, same halo convention.
5. Compare the two totals against each other and against step 3. State which grouping wins, and at what ratio of fixed per-transfer cost to per-value cost the answer would flip. Instantiate a machine on each side of that ratio, or state that no realizable machine sits on one side of it and why. The transfer counts from steps 3 and 4 are what make that ratio computable, and volumes alone will not. Computation is identical across the two groupings because Setup fixed them to the same group count and equal areas, so it cancels and cannot decide between them.
6. Place the groups, balancing work and co-locating heavy communicators. Then double the execution unit count, say which steps would have to be redone, and give the numbers those steps produce at the doubled count.

...
## Instructions
1. Write down the mapping: for a thread with a given index, which record and which field does it touch, and which branch does it take.
...
3. For the same group and one instruction, list the addresses touched and count how many distinct fixed-size aligned units they fall into. That count is the transaction cost.
...
## Success Check
- Both costs are reported as counts derived from the mapping, not as impressions of whether the code looks parallel.

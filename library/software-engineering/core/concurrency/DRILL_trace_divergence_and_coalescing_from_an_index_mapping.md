...
3. Declare the size of the aligned memory unit your hardware model serves. The counts depend on it, so it has to be fixed before you count rather than assumed. For the same group and one instruction, list the addresses touched and sort them into units of that size, counting how many distinct units they fall into. That count is the transaction cost.
4. Change only the mapping — assign each thread a contiguous run of records instead of a strided one, or the reverse — and redo steps 2 and 3. Record both numbers for both mappings, and state the relationship between them.
5. Reorganize the records into one array per field, leaving the mapping alone, and redo step 3. Evaluate this change separately from the mapping change rather than applying both together.
6. State which single change bought the most, and whether the control-flow cost and the memory cost moved together or in opposite directions. If the branch cost did not move between the mappings, name the branch distribution as the fault; if the memory cost did not move, name the record size against the declared unit size. Either way, name the value that would move it, and do not retune an input until the expected result appears.

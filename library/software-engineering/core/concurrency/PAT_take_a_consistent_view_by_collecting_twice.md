...
- Use the same construction to establish a *predicate over participants*, not only a snapshot of data. Deciding that a distributed computation has finished — that every worker is simultaneously idle — is the standard case, and it cannot be done by counting announcements: a worker that reported itself idle can be made active again by one that has not, so idleness is not a state that accumulates. What has to be established is that the predicate held at one instant, which is exactly what two matching passes over a change indicator give you.
...
- Don't make the indicator wider than the lag the protocol permits, either — the required width is exactly that bound. Where a protocol guarantees nobody can fall more than one round behind, a single alternating bit distinguishes rounds completely, which is why a reusable rendezvous can flip a sense flag rather than carry a counter. Where nothing bounds the lag, no fixed width is safe and the counter has to be sized against the longest interval any participant might sleep through.
- Don't reuse a shared object by resetting its state to the starting value. A fast participant can finish the next round and arrive again before a slow one has observed the previous round ending, and the reset then makes the two rounds indistinguishable — the slow participant waits forever on a condition that already came and went. Distinguishing consecutive rounds, rather than restoring the object to how it began, is what makes reuse safe.

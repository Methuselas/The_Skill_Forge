...
- Read the guarantees on two axes rather than as one ladder, because the ladder hides which of them you are paying for. The first axis is how many threads are promised progress: every thread that keeps taking steps, or merely some thread. The second is whether the promise stands unconditionally or rests on an assumption about the scheduler. Wait-free and lock-free promise progress under any schedule whatsoever and differ only on the first axis; the rest all buy their simplicity by assuming something about how threads are run.
- Know the third nonblocking option, which sits between the other two in cost and is routinely overlooked. An operation is obstruction-free if it completes in a bounded number of steps once it runs without interference — nothing is promised while conflicting operations are in flight, but no thread's delay can block another. It is far easier to implement than the unconditional guarantees because it may abandon and retry freely, and it is paired with something that reduces conflict: backing off on detecting one is the simplest such arrangement. Note what the assumption actually requires — not that other threads pause, only that the ones touching the same object do.
- Frame the choice as what you are willing to assume about the platform, since that is what actually separates these. Every guarantee assumes the thread is eventually scheduled at all. Beyond that, the blocking conditions assume the scheduler is fair to every thread with work pending, obstruction-freedom assumes conflict eventually subsides, and the unconditional ones assume nothing further and pay for it. For an operation whose completion is genuinely critical, refusing the extra assumptions is worth real overhead; for most operations it is not.
- Choose per operation rather than per object, because one object's methods routinely warrant different answers. A lookup on a hot path that must never stall can be built to complete unconditionally, while the rarely called update to the same structure is guarded by an ordinary lock — and that combination is a normal design rather than an inconsistency.
...
- Don't reject a blocking design on the strength of the classification alone. What makes blocking hurt is a thread being descheduled inside a critical section, so where that is rare the blocking and nonblocking conditions are close to indistinguishable in practice. The classification tells you the worst case; whether the worst case is reachable often enough to matter is a question about the platform and the load.
...
## Checklist
- Which progress guarantee does this design actually provide — to every thread or only to some, and under what assumption about the scheduler?
- What is this operation allowed to assume about the platform, and is that assumption written down?
- Do all of this object's operations need the same guarantee, or is one of them on a critical path the others are not?

...
- Carry a counter alongside the pointer and compare both, which is the cheapest remedy and the one to reach for first. Increment it on every update, and the conditional update then fails whenever the location has been written since you read it — even if it was written back to the value you saw. This converts "unchanged" into "untouched," which is the property the reasoning actually needed. On a sixty-four-bit machine the counter can usually be packed into spare bits of the pointer itself so the pair still updates in one instruction; where it cannot, the pair needs a level of indirection and the cost rises accordingly.
- Size that counter so it cannot wrap while any operation is in flight. It is the same failure one level down — a counter that returns to a previously observed value makes a modified location look untouched — and it is usually dismissed as impossible rather than checked.
...
- Publish what you are using, where you want reclamation without quiescence. Each thread claims a slot it alone writes and stores into it the address it is about to dereference, clearing it when done; a thread ready to free something scans every slot first and frees only what no slot names, deferring the rest to a retire list. This is what a hazard pointer is, and its appeal is that a reader announces its interest with a single store instead of coordinating with anyone. Its costs are a fixed pool of slots that can run out, and a scan proportional to the pool on every reclamation attempt.
- Re-read after publishing, because publishing an address does not by itself protect it. Between reading a pointer and storing it into your slot, the thing it names can be unlinked and freed — so the sequence has to be publish, then confirm the pointer is still installed where you found it, and start over if it is not. A published address that was already dead protects nothing, and this loop is what makes the scheme correct rather than merely plausible.
- Weigh the per-operation cost against the precision, since that is the decision between the two main schemes. Publishing each address you touch bounds the garbage tightly and costs a strongly ordered write for every location an operation visits — and it is the fences, not the writes, that dominate. Tracking only whether each thread is *inside* an operation costs two ordered writes for the whole operation regardless of its length, and gives up any bound on how much unreclaimed memory can accumulate. Frequent removals and threads that may stall argue for the first; rare removals, or an environment where an operation cannot be suspended partway, argue for the second.
- Detect quiescence by watching for a *change* rather than for an idle moment, which is what makes the cheap scheme practical. Have each thread advance a counter on entering an operation and again on leaving, so an even value means it holds no references. A thread under constant load may never be observed even — but observing it move from one odd value to a different odd value is just as good, because it has finished one operation and begun another, and anything unlinked during the first cannot be reached by the second.
- Scan on a threshold rather than on every removal, whichever deferral scheme you use. Reclaiming after each operation makes every operation pay a full traversal of the pending list; waiting until the list is some multiple of the structure's own size amortizes that and guarantees a worthwhile proportion is collectable when you do scan. The multiple is the knob between memory held and throughput given up.
...
- Give each thread its own free list when recycling nodes in a lock-free structure, since a private list needs no synchronization at all to take from or return to. The scheme assumes each thread performs roughly as many acquisitions as releases; where that balance does not hold, one thread's list drains while another's grows without bound, and the fix — periodically taking nodes from another thread's list — reintroduces exactly the synchronization the private list was avoiding.
...
- Don't adopt the cheap scheme where a thread can stall inside an operation. One thread suspended partway through leaves every other thread unable to reclaim anything, forever — the garbage is unbounded by construction, and the usual mitigation is not a smarter scheme but a context where suspension cannot happen or removals are rare enough not to matter.
- Don't gate reclamation on there being exactly one operation in flight. Counting active operations and freeing only when the count says you are alone is the obvious cheap scheme and it fails in the direction that matters: under the sustained contention that made a lock-free structure worth building, the count never falls that far, nothing is ever freed, and the pending list grows without bound. A scheme whose collection stops precisely when load is highest is a leak with a quiet period.
...
- If addresses are published before use, is the pointer re-checked after publishing?
- How many ordered writes does one operation perform, and does that scale with its length?
- Can a thread stall inside an operation, and what does that do to reclamation?

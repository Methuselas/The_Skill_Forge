...
- Decide how searching ends. A worker that finds every queue empty will otherwise search forever, long after all the work is done, so the design needs a way to establish that there is genuinely nothing left rather than merely nothing found this time.

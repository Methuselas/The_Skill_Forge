...
- Order the liveness guarantees rather than treating them as a list, because each is strictly weaker than the last and it is easy to claim a stronger one than you hold. Some thread always completes is the floor. Every thread eventually completes is stronger and does not follow from it. Threads complete in the order they arrived is stronger again, does not follow from either, and is the only one that says anything about how long any particular thread waits.
...
- Don't settle for a definition of the floor that only rules out everything stopping. Threads that keep taking steps while undoing each other's progress are moving, are consuming processor, and are completing nothing — a system in that state passes any check that looks for blocked threads, and the useful requirement is that some thread actually finishes.

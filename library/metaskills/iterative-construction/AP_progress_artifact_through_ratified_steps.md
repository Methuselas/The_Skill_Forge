...
5. **Evaluate the returned candidate against two things.** Activate `PAT_verify_result_against_objective_after_production`. First, did it accomplish the current step's job without exceeding its information ceiling? Second, did it preserve every upstream commitment that the current step was not authorized to reopen?
...
15. **Stop when the thread's final step is complete.** Completion means the last step achieved its result while all prior commitments remain valid, confirmed by `PAT_verify_result_against_objective_after_production` against the thread's stated objective rather than against process compliance. Do not keep elaborating merely because more detail, polish, or alternatives are possible.

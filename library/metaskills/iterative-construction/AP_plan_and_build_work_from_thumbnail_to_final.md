...
## Notes
Visual production loads whatever reorientation Pattern its own domain supplies for returning to the picture's governing centerline. In direct render mode, Stages 0–3 are reasoning responsibilities, not fictional intermediate artifacts. In explicit staged production, `AP_progress_artifact_through_ratified_steps` owns progressive ratification and the active domain supplies the approval-gated stage AP thread. This metaskill names neither, because it ships into releases that may not contain them. The current image operation receives only its current stage job rather than the whole workflow as visual content.
